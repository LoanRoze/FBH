...
export async function createMission({ mission_type, mission_location, mission_date, mission_rank }) {
    if (!mission_type || !/^[a-zA-Z ]+$/.test(mission_type)) {
        throw new BadRequestError("Type de la mission non valide (pas de charactères spéciaux)");
    }
    if (!mission_location || !/^[a-zA-Z ]+$/.test(mission_location)) {
        throw new BadRequestError("Lieu de la mission non valide (pas de charactères spéciaux)");
    }

    if (!mission_rank || !mission_rank in Object.keys(RANKS)) {
        throw new BadRequestError("Rank de la mission non valide (S+, S, A, B, C)");
    }
    const dateRegex = /^\d{4}-(0[1-9]|1[0-2])-(0[1-9]|[12]\d|3[01])$/;
    if (!dateRegex.test(mission_date) || !new Date(mission_date).toISOString().slice(0, 10) === mission_date) {
        throw new BadRequestError("Date de la mission non valide (format YYYY-MM-DD attendu)")
    }

    const mission = await MissionRepository.createMission({ mission_type, mission_location, mission_date, mission_rank });
...
export async function assignHeroToMission(hero_id, mission_id) {
    ...
    if (hero.hero_rank[1] < mission.mission_rank[1]) {
        throw new BadRequestError("le rank du héro est trop faible pour accepter la mission")
    }

    const mission_hero = await assignHeroToMission(hero_id, mission_id)
    return mission_hero
}
...
export async function updateMission(mission_id, { mission_type, mission_location, mission_date, mission_rank }) {
    const mission = await getMissionById(mission_id)
